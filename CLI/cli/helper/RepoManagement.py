...
class RepoManagement():
    ...
    def __init__(self, config_folder):
        ...
        '''
        self.__repo_config_file = os.path.join(
            config_folder, "repo_config.json")
        try:
    ...
    def get_branch_data(self, branch_name=None, branch_id=None):
        ...
        '''
        target_branch = None if not branch_id else self.__repo_config[
            ...
            if branch['name'] == branch_name:
                ...
                for commit in branch['commits'].values():
                    t1 = datetime.strptime(
                        latest_commit['date_created'], '%Y-%m-%dT%H:%M:%S.%fZ')
                    t2 = datetime.strptime(
                        commit['date_created'], '%Y-%m-%dT%H:%M:%S.%fZ')
                    if t2 > t1:
    ...
    def update_commits(self, new_commits, branch_id):
        try:
            with open(self.__repo_config_file, 'w') as f:
                commits = dict(self.get_repo_config()[
                               "branches"][f"{branch_id}"]["commits"])
    ...
    def create_branch(self, branch_data):
        try:
            with open(self.__repo_config_file, 'w') as f:
                ...
                self.__repo_config['branches'][f'{branch_id}']['commits'] = {
                    f'{last_commit_id}': last_commit}
                json.dump(self.__repo_config, f)
        except:
            raise Exception("Error, cannot open repo_config.json")
    ...
    def get_largest_commit(self, commit1, commit2):
        t1 = datetime.strptime(
            commit1['date_created'], '%Y-%m-%dT%H:%M:%S.%fZ')
        t2 = datetime.strptime(
            commit2['date_created'], '%Y-%m-%dT%H:%M:%S.%fZ')
        if t2 > t1:
            return commit2
        return commit1
