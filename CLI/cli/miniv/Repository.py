...
class repo():
    ...
    def login(self):
        ...
        if response.status_code != 200:
            ph.msg(
                ph.red("You may entered the password or the email wrong, please try again!"))
            email = str(input("Email: "))
            password = str(getpass(prompt='Password: '))
            response = requests.post(
                'http://127.0.0.1:8000/api/v1/login/',
        ...
        if response.status_code == 200:
            ...
            user_data['password'] = UM.UserManagement.encrypt_password(
    ...
    def create_user_configuration(self, user_data):
        ...
        user_config_file = os.path.join(
            self.__config_folder, "user_config.json")
        if not os.path.isfile(user_config_file):
            try:
                ...
                raise Exception(
                    "Internal error, couldn't create user_config file!")
        else:
    ...
    def clone(self):
        ...
        if not os.path.isfile(repo_config_file):
            try:
                ...
                raise Exception(
                    "Internal error, couldn't create repo_config file!")
        else:
            raise Exception(
                "Error, there exists already repository config file!")
        ...
        commit_folder = os.path.join(
            branch_folder, os.listdir(branch_folder)[0])
