...
def commit_parser(subparsers):
    ...
    list_group.add_argument(
        '-l', '--list', help='list commits', action='store_true')
    ...
    create_group.add_argument(
        '-c', '--create', help='Commit message', dest='create')

    amend_group = parser.add_argument_group('amend commit')
    amend_group.add_argument(
        '-a', '--amend', help='Amend the previous commit', action='store_true')
...
def branch_parser(subparsers):
    ...
    list_group.add_argument(
        '-l', '--list', help='list local branches', action='store_true')
    list_group.add_argument(
        '-lr', '--remote', help='list remote branches in addition to local branches', action='store_true')

    create_group = branch_parser.add_argument_group('create branches')
    create_group.add_argument('-c', '--create', nargs='+',
                              help='create branch(es)', dest='create_b', metavar='branch')

    delete_group = branch_parser.add_argument_group('delete branches')
    delete_group.add_argument('-d', '--delete', nargs='+',
                              help='delete branch(es)', dest='delete_b', metavar='branch')
    ...
    get_current_group.add_argument(
        '-cu', '--current', help='get the current branch', action='store_true')
...
def merge_branches_parser(subparsers):
    ...
    parser.add_argument(
        '--branch', help='the branch name that is going to be merged into the current branch', dest="branch")
...
def checkout_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        'checkout', help=desc, description=(desc.capitalize()), aliases=['ch'])
    parser.add_argument(
        '--to', help='The branch destination where to switch to!', dest='branch_name')
...
def diff_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        'diff', help=desc, description=(desc.capitalize()), aliases=['df'])
    parser.add_argument(
        '--of', help='The file name to apply diff to!', dest='file1')
    parser.add_argument(
        '--diff_repo', help='Get the diff of the whole repository!', action='store_true')
...
def undo_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        'undo', help=desc, description=(desc.capitalize()), aliases=['ud'])
